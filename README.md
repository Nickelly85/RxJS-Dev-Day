...
Create a new Observable to assign your http request observable to.  
...
In RxJS Subjects are both observers and observables, meaning you can emit values into them using `next()` and also subscribe to receive those values. 
On this page, the setup simulates two separate components. You will use a `Subject`to emit values from the `userService.getUsers()` method, which returns a 
...
- If no double click occurs trigger `userService.getRandomUsers()` and `userService.getUser()` merging the two streams together and combining the 
data into one array. 

### Double Click
- When a user performs a double click:
  - Trigger `userService.getRandomUser()` to get one random user 
  - Add this user to the existing users array 
  - Ensure this behaviour doesn't trigger the single-click event.

### Backspace Event
- When a user presses the `Backspace` on their keyboard clear the
users list. 
